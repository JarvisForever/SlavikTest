...
def start_message(message):
    bot.send_message(message.chat.id, 'Приветствую тебя, высшая форма жизни, я бот Славик, который подкажет тебе расписание уроков и не только.\nКоманды, которыми ты можешь воспользоваться:\n/homework-доманее задание;\n/event-какие-то событие, которое должно или будет происходить;\n/timetable-расписание уроков и классы, где будут проходить уроки;\n/clock-расписание звонков;\n/menu-Вернуться назад к командам;\n(Некоторые функции могут добавляться со временем, и ты можешь подказать какую ту функцию добавить, а мой создатель подумает над этим)')
